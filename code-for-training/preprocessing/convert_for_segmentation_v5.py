...
def load_single_image(dataset, subject_id, visit, side, input_dicom, input_points,
...
#                  f'forced pixel spacing given, but input file already contains pixel spacing, {source_pixel_spacing} != {forced_input_pixel_spacing}'
    ...
    smooth_curves = {
        key: make_smooth_contour(curves[key], smoothness=1, num=100)
        for key in curves.keys()
    }

    regions = [
        ('background', np.array([
            # left of lateral end of proximal femur
            [(0 if side == "right" else img_pixels.shape[1]),
             curves[f'{side} proximal femur'][0][1]],
            # top left
            [(0 if side == "right" else img_pixels.shape[1]), 0],
            # top right
            [(0 if side == "left" else img_pixels.shape[1]), 0],
            # right of medial end of proximal femur
            [(0 if side == "left" else img_pixels.shape[1]),
             curves[f'{side} proximal femur'][0][1] - 1],
            # lateral end of proximal femur
            [curves[f'{side} proximal femur'][0][0],
             curves[f'{side} proximal femur'][0][1] - 1],
        ])),
        ('sourcil', np.array([
            # 2 mm above the sourcil
            *smooth_curves[f'{side} sourcil'],
            *(smooth_curves[f'{side} sourcil'][::-1, :]
              - ([0, 2] / np.array(pixel_spacing))),
        ])),
        ('joint space', np.array([
            # sourcil
            *smooth_curves[f'{side} sourcil'],
            # down from lateral point of sourcil
            [smooth_curves[f'{side} sourcil'][-1][0],
             curves[f'{side} posterior wall'][2][1]],
            # follow posterior wall (to be covered by femoral head)
            curves[f'{side} posterior wall'][2],
            curves[f'{side} posterior wall'][1],
            curves[f'{side} posterior wall'][0],
        ])),
        ('femur', np.array([
            # proximal femur
            *smooth_curves[f'{side} proximal femur'],
        ])),
    ]
...
def write_single_image(h5, dataset, subject_id, visit, side, input_dicom, input_points,
